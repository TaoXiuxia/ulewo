ulewo.url = {
	...
	loadCurDaySignInUrl : "loadCurDaySignIn",
...
function loadCurDaySignin(page) {
	...
	ulewo.ajaxRequest({
		async : true, url : ulewo.url.loadCurDaySignInUrl, showLoad : false, data : {pageNo : page}, fun : function(res) {
			$("#loading").remove();
			$("#curDaySignIn").empty();
			var list = res.data.list;
			if (list.length == 0) {
				$("<div class='no-data'>今天还没有人签到</div>").appendTo($("#curDaySignIn"));
			}
...
function setSignInInfo(res) {
	...
	if ($(".no-data").length == 0) {
		new SignInItem(res.data).appendTo($("#curDaySignIn"));
	} else {
		$(".no-data").remove();
		new SignInItem(res.data).appendTo($("#curDaySignIn"));
	}
...
function loadMySignInInfo(year) {
	ulewo.ajaxRequest({
		...
		fun : function(res) {
			...
			var select = $("<select></select>").appendTo(selectCon).change(function() {loadMySignInInfo($(this).val());});
