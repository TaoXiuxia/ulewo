ulewo.url = {
	...
	loadCurDaySignIn : "loadCurDaySigin",
...
function loadCurDaySigin(page) {
	ulewo.curPage = page;
	$(
			'<div id="loading"> <div class="loading-con"><img src="'
					+ ulewo.absolutePath
					+ '/resource/images/loading.gif"/><span>正在加载.......</span></div></div>')
			.appendTo($("#curDaySigin"));
	ulewo.ajaxRequest({
		async : true,
		...
		fun : function(res) {
			...
			if (list.length == 0) {
				$("<div class='no-data'>今天还没有人签到</div>").appendTo(
						$("#curDaySigin"));
			}
...
function SignInItem(data) {
	...
	$(
			"<div class='signIn-item-img'><a href='" + ulewo.absolutePath
	...
	$(
			"<div class='signIn-item-name'><a href='" + ulewo.absolutePath
					...
					+ "</a></div>").appendTo(info);
	$("<div class='signIn-item-time'>" + data.signTime + "</div>").appendTo(
			info);
	...
	ulewo
			.ajaxRequest({
				...
				fun : function(res) {
					...
					var selectDiv = $("<div class='selectDiv'></div>")
							.appendTo(userSignInPanel);
					var selectCon = $("<div class='selectCon'></div>")
							.appendTo(selectDiv);
					var select = $("<select></select>").appendTo(selectCon)
							.change(function() {
					...
					for (var i = 0, length = months.length; i < length; i++) {
						var divCon = $("<div class='calendarCon'></div>")
								.appendTo(userSignInPanel);
						var table = $(
								"<table cellpadding='1' cellspacing='1'></table>")
								.appendTo(userSignInPanel).appendTo(divCon);
						$(
								"<tr><td colspan='8' class='month'>" + (i + 1)
										+ "月</td></tr>").appendTo(table);
						$(
					...
					$("<div style='clear:left'></div>").appendTo(
							userSignInPanel);
				}
			});
}